...
self._itemCounter=0;m_helper.each(items,function(e){e.multiselectElement.parentElement.parentElement.classList.remove('active');self._item.options[e.index].removeAttribute('selected');m_helper.uncheck(e.multiselectElement)});return!1},_updateSelectAll:function(self){var allChkBox=document.getElementById(self._getItemListIdentifier()).querySelector('input[type=checkbox]');if(self._itemCounter==self._getItems().length){allChkBox.checked=!0}
...
return res.length==1?res[0]:$(res)};$.fn.multiselectDestroy=function(){if(!window.multiselects._items)return;var iiii=this.call(this.caller);console.log("IIII",iiii);if(this.length!=0){$(this).each(function(i,e){var index=window.multiselects._items.indexOf(e);if(index!=-1){window.multiselects[index].destroy();window.multiselects._items.splice(index,1)}})}};$(document).click(function(event){hideMultiselects(event)})}else{document.multiselect=function(selector){var res=[];if(!window.multiselects._items){window.multiselects._items=[]}
...
document.multiselectDestroy=function(selector){}
